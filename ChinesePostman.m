%% Modified Chinese Postman Problem Function
...
%
% Solves the (modified) Chinese Postman Problem on an undirected graph.
% Eulerian graphs are traversed directly with Fleury's algorithm.
% Non-Eulerian graphs are made Eulerian by matching odd-degree nodes via
% integer linear programming (intlinprog), then traversed with Fleury.
%
% INPUTS:
%   ADJ          - Binary adjacency matrix (0/1).
%   Matrix_Input - ADJ * DIST (weighted adjacency).
%   DIST         - Pairwise distance matrix.
%   Start        - Starting node index.
%   ee           - Edges to exclude from the LP matching.
%   cl           - Closest node to the robot (preferred start).
%
% OUTPUTS:
%   Path   - Node sequence of the traversal.
%   Weight - Total path weight.
%   add    - Edges added to make the graph Eulerian.
%   st     - Starting edge (largest connector removed to open the circuit).
...
switch t
    ...
    otherwise
        ...
        if isempty(Added_Edges)
            rmflag=false;  % Retry including crack edges in LP
            [Added_Edges,Added_Weight,st] = optOddPair(ADJ,DIST,ee,cl,flag,rmflag);
        end
        ...
        if size(C,2)>1
            % Reconnection candidates: odd-degree nodes only, avoiding mid-crack degree-2 nodes.
...
switch nargin                           %Switch for number of inputs
    case 3                              %Fleury's adjusted with Shortest Path
        lP = P;                         %Added edges from Shortest Path in List Format
    m=max(max(A(R,1:n)));
    for p = m:-1:1
    ...
    if ~isempty(con)
        for C = con
            if A(R,C)> 0                    %Runs if the edge exists
                ...
                if Cval == 1 && (sum(A(:,C))>1 || sum(sum(sum(A))==2))  %Runs if A remains connected
                    if T==0
                    A(R,C)= A(R,C)-1;              %Removes the edge from A
                    if d == 0               %Undirected Graph
                        A(C,R)= A(C,R)-1;          %Removes back edge from A
                    end
                    end
                    break
                end
    ...
    for C = 1:n                         %Runs while unchecked columns remain
        if A(R,C)> 0                    %Runs if the edge exists
            T = 0;
            Cval = FleuryConn(A,R,C,T);
            if Cval == 1                %Graph remains connected with edge removed
                A(R,C)= A(R,C)-1;              %Removes the edge from A
                if d == 0               %Undirected Graph
                    A(C,R)= A(C,R)-1;          %Removes back edge from A
                end
...
if ~T
    A(R,C)=A(R,C)-1;                               %Sets element to zero
    if d == 0
        A(C,R)=A(C,R)-1;                           %For undirected graphs
    end
end
n=size(A,1);                            %Number of Vertices
B = zeros(n,n);
...
A  = A(l,l);                            %Matrix is adjusted to only contain active vertices
end
%--------------------------------------------------------------------%
...
%--------------------------------------------------------------------%
%Optimum Pairings of Odd Degree Vertices (legacy brute-force)
%--------------------------------------------------------------------%
...
%--------------------------------------------------------------------%
function [Added_Edges,Added_Weight,st] = optOddPair(A,DIST,ee,cl,flag,rmflag)
% Pairs odd-degree nodes via integer linear programming (intlinprog).
...
%--------------------------------------------------------------------%
function [Added_Edges] = linearProg(G,b_n,DIST,ee,flag,rmflag)
% Solves the minimum-weight perfect matching via intlinprog.
...
function [Added_Edges] = linearProgRRP(G,b_n,combEdge,DIST,ee)
% Variant of linearProg for the Rural Postman sub-problem.
...
u = size(K,1);                  %Number of edges traversed that don't exist
if u==0
