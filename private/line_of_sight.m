%% Parametric ray-cast visibility check between observer and target nodes
...
% line_of_sight  Parametric ray visibility check between observer and target.
%   visibility = line_of_sight(observer_state, current_target_node, external_boundaries)
%   Returns a binary vector indicating whether each target node is directly
%   visible from the corresponding observer position within the boundary polygon.

%Dissect observer state into coordinates
x_observer = observer_state(:,1);
y_observer = observer_state(:,2);

%Dissect current target node coordinates
...
function p=loopFun(x_1,y_1,x_2,y_2,f)
    ...
    [x_observer,y_observer,x_current_target,y_current_target,beam_direction_vector]=f{1}();
