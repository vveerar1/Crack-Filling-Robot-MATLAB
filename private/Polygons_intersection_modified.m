function [Geo] = Polygons_intersection_modified(S,Display_result,Accuracy)
...
% 
% This function uses Polygon Clipper, a function posted by Sebastian H�lz:
% 
...
% Compute pairwise intersections for all polygon pairs that overlap.
[i,j]=find(triu(overlaps([S(1:end).P]),1));
for n = 1:length(i)
    Geo(n).index  = [i(n),j(n)];
    Geo(n).P      = intersect([S(i(n)).P],[S(j(n)).P]);
    Geo(n).area   = Geo(n).P.area;
    Res(i(n),j(n)) =Res(i(n),j(n)) + Geo(n).area;
end
...
for i=1:numel(Index2)
    if ~isempty(Geo(V(i)).P)
        P = intersect(Geo(V(i)).P,S(Index2(i).data(end)).P);
        if ~isempty(P)
            area = 0;
            for p = 1:size(P,2)
                area = area + P(p).area; %(1-2*P(p).hole)*polyarea(P(p).x,P(p).y);
...
Geo(unique(To_delete))=[];

% Now that all intersection polygons have been computed, split the original
% polygons into non-overlapping sub-regions.
Geo = Polygons_intersection_Posttreatment_modified(S,Geo,Display_result,Accuracy);
