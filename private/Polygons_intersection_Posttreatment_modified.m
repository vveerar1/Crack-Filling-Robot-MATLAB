function Geo = Polygons_intersection_Posttreatment_modified(S,Geo,Display_result,...
                                                                  Accuracy)
...
Geo   = [S,Geo];
Geo   = Polygons_intersection_Polygon_cleanup_modified(Geo,Accuracy);
N_Geo =  numel(Geo);
...
%                 P1 = polyshape(Geo(Mod(k)).P.x,Geo(Mod(k)).P.y);%P1.plot;hold on
%                 P2 = polyshape(Geo(Ref(j)).P.x,Geo(Ref(j)).P.y);%P2.plot;hold off
                P = subtract(Geo(Mod(k)).P,Geo(Ref(j)).P);
                
%                 if ~isscalar(P)%~all(class(P)=='polyshape')
%                    disp(class(P)) 
%                 end
...
%                 if ~isempty(P3)
%                     for p = 1:size(P3,2)
%                         P(p).x = P3(p).Vertices(:,1);
%                         P(p).y = P3(p).Vertices(:,2);
%                         P(p).hole = P3(p).NumHoles;%0;
%                     end
%                 else
%                     P={};
%                 end
                
                P = Polygons_intersection_Polygon_cleanup_modified(P,Accuracy);
                if ~isempty(P)
                    Geo(Mod(k)).P = P;
                else
...
[Geo, Geo_area] = Polygons_intersection_Compute_area_modified(Geo);
N_Geo = numel(Geo);
To_delete = [];
for i=1:N_Geo  
    for j=1:numel(Geo(i).P)
...
if Display_result
    ...
    for i=1:N_polygons
        ...
        h(i) = plot(S(i).P(j),'FaceColor',colour(i,:),'LineWidth',3);
%         text(S(n).x(1)+0.2,S(n).y(1)+0.2,num2str(n),...
%              'color',colour(n,:),'FontSize',14);
            if S(i).P(j).NumHoles>0
                set(h(i),'LineStyle','--');
            end
        end
...
%                   temp,'FaceAlpha',0.3)
            plot(Geo(i).P(j),'FaceColor',temp,'FaceAlpha',0.3)
            [geom(1),geom(2)]=centroid(Geo(i).P(j));
            Str='';
            ...
            if inpolygon(geom(1),geom(2),...
                         [Geo(i).P(j).Vertices(:,1);Geo(i).P(j).Vertices(1,1)],...
                         [Geo(i).P(j).Vertices(:,2);Geo(i).P(j).Vertices(1,2)])
                X1 = geom(1);
                Y1 = geom(2);
            else
                temp1 = axis;
                X1 = min(Geo(i).P(j).Vertices(:,1)) + (temp1(1)+temp1(2))/100;
                Y1 = min(Geo(i).P(j).Vertices(:,2)) + (temp1(3)+temp1(4))/100;
            end
    ...
    for i=1:N_polygons
        for j=1:numel(S(i).P)
        h = plot(S(i).P(j),'FaceColor',colour(i,:),'LineWidth',3);
%         text(S(n).x(1)+0.2,S(n).y(1)+0.2,num2str(n),...
%              'color',colour(n,:),'FontSize',14);
            if S(i).P(j).NumHoles>0
                set(h,'LineStyle','--');
