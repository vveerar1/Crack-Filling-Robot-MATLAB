function Geo = Polygons_intersection_Posttreatment_modified_clipper(S,Geo,Display_result,...
                                                                  Accuracy)
...
% g=@(P) [P.x,P.y;NaN,NaN];
g=@(P) [P.x,P.y;P.x(1),P.y(1)];

G=@(P) cell2mat(vertcat((arrayfun(g,P,'UniformOutput',0)))');
...
%                 P = PolygonClip(Geo(Mod(k)).P,Geo(Ref(j)).P,0);
                [px,py] = polyclip(G(Geo(Mod(k)).P),G(Geo(Ref(j)).P),0);
%                 if ~isempty(px);P=cell2struct([px;py],{'x','y'},1)';[P(:).hole]=deal(0);else;P = struct('x',{},'y',{},'hole',{});end
                if ~isempty(px);P=cell2struct([px;py;cellfun(@(x,y)ispolycw(x,y),px,py,'UniformOutput',0)],{'x','y','hole'},1)';else;P = struct('x',{},'y',{},'hole',{});end
...
function P_out=cellPolyCom(P_in)
g=@(P) [P.x,P.y;P.x(1),P.y(1)];%];
G=@(P) cell2mat(vertcat((arrayfun(g,P,'UniformOutput',0)))');P_out=[];
for i=1:length(P_in)
    P_out=[P_out;G(P_in(i))];
%     if i<length(P_in);P_out=[P_out;NaN NaN];end
end